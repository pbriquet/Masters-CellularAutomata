                                ...
                                for(RunCAinVFLine){
                                        for(RunCAinVFCol){
                                                ...
                                                if( fabs( CaP.state ) >= 2){
                                                                ...
                                                                #ifdef fSStateFunction
                                                                        #ifndef esiCluster
                                                                        #ifdef ScheilMicrofS  // Calcula a fração de sólido local pela equação de Scheil
                                                                        CaP.fS = fSScheil(TCA);
                                                                        #endif
                                                                ...
                                                                #ifdef esiCluster
                                                                        CaP.fS = clusters[ClusterP].fS;
                                                                        if(clusters[ClusterP].fixed == 0){
                                                                                if( TCACalculated == 0)
                                                                                        TCA = getTThermoCouple(t, CAPyPosition, ThermoData);
                ...
                for(RunClusterMatrix){
                        ...
                        if(clusters[ii].cluster == ii ){
                                ...
                                if(clusters[ii].size < 0)
                                        ii=ii;
                                #ifdef fSStateFunction           // Ajusta o cálculo de fS da cluster para o tamanho
                                        #ifndef esiCluster
                                        ...
                                        if(clusters[ii].fixed == 0){
                                                ...
                                                clusters[ii].Cdt = clusters[ii].Cdt/clusters[ii].size;
                                                clusters[ii].dCddt = clusters[ii].dCddt/clusters[ii].size;
                                                //clusters[ii].ve = clusters[ii].ve/clusters[ii].countint;
                                                clusters[ii].ve = clusters[ii].ve/clusters[ii].size*2.0/pow(M_PI, 1.0/3.0);
                                                clusters[ii].esidt = esidtGeral(&clusters[ii]);
                                                if( ii == 91){
                                                        fprintf(Teste91, "%e;%e;%e;%e;%e;%e;%e;%e;%e;%e\n", t, clusters[91].fS, clusters[91].esidt, clusters[91].Cdt,  clusters[91].dCddt, clusters[91].ve, clusters[91].Ae, clusters[91].Ve, clusters[91].de, clusters[91].d[1]);
                                                }
