...
int main()
{
                ...
                */
                ...
                printf("\n\nbetad = %f", fbetad(esi,phie,Senv,Ssol));
                printf("\nbetal = %f", fbetal( fbetad(esi,phie,Senv,Ssol),el,phie) );
                printf("\nKv = %f", fKvpartition(el,fbeta(fbetad(esi,phie,Senv,Ssol),fbetal( fbetad(esi,phie,Senv,Ssol),el,phie),el),fbetad(esi,phie,Senv,Ssol)));
                ...
                printf("\n\nKv Partition Test");
                printf("\n\nKv = %f", fKvpartition( el, fbeta( fbetad(esi, phie, Senv, Ssol) , fbetal(fbetad(esi, phie, Senv, Ssol), el, phie), el), fbetad(esi, phie, Senv, Ssol) ));
                //printf("\nbeta = %f", fbeta( 100.0, fbetal( 100.0, el, phie), el ) );

                for(el = 0.001; el < 0.999; el+=0.001){
                                fprintf(Kv,"\n%e;", el);
                                esi = 0.01;
                                fprintf(Kv, "%f;",  fKvpartition(el,fbeta(fbetad(esi,phie,Senv,Ssol),fbetal( fbetad(esi,phie,Senv,Ssol),el,phie),el),fbetad(esi,phie,Senv,Ssol)) );
                                esi = 0.2;
                                fprintf(Kv, "%f;",  fKvpartition(el,fbeta(fbetad(esi,phie,Senv,Ssol),fbetal( fbetad(esi,phie,Senv,Ssol),el,phie),el),fbetad(esi,phie,Senv,Ssol)));
                                esi = 0.4;
                                fprintf(Kv, "%f;",  fKvpartition(el,fbeta(fbetad(esi,phie,Senv,Ssol),fbetal( fbetad(esi,phie,Senv,Ssol),el,phie),el),fbetad(esi,phie,Senv,Ssol)));
